...
static uint8 scanRspData[] =
{
  ...
  'L','I','V','O','L','O','_','b','l','e','_','s','w','_','1','.','0',
...
static uint8 advertData[] =
{
  ...
  GAP_ADTYPE_SERVICE_DATA,//GAP_ADTYPE_16BIT_MORE,      // some of the UUID's, but not all
  0XF1,//LO_UINT16( SIMPLEPROFILE_SERV_UUID ),
  0XFF,//HI_UINT16( SIMPLEPROFILE_SERV_UUID ),
  0x11,//0x11,0x00,
};

// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "LIVOLO";
...
//------------------------------------------------------------------------------
  ...
  P0DIR = 0xF8; // Port 0 pins P0.0 and P0.1 as input (buttons),
                // all others (P0.2-P0.7) as output
  P1DIR = 0xFF; // All port 1 pins (P1.0-P1.7) as output
  P2DIR = 0x1F; // All port 1 pins (P2.0-P2.4) as output

  P0 = 0x07; // All pins on port 0 to low except for P0.0 and P0.1 (buttons)
 ...
 * @return  none
 */
static void simpleBLEPeripheral_ProcessOSALMsg( osal_event_hdr_t *pMsg )//obrabotka sobytiy
{
  switch ( pMsg->event )
  {     
...
*/    
  ...
  if ( keys & HAL_KEY_SW_2 )
  {
    ...
    if( gapProfileState != GAPROLE_CONNECTED )
    {
      ...
      {
        new_adv_enabled_status = TRUE;// FALSE;
      }
    ...
    case GAPROLE_ADVERTISING_NONCONN:
      {
        ...
        GAPRole_SetParameter(GAPROLE_ADV_NONCONN_ENABLED, sizeof(uint8),&advertEnabled);
 ...
 *
 * @return  none
  To use P0.6 as output, with a high value, please do the following:
  P0SEL &= BIT6; // GPIO.
  P0DIR |= BIT6; // Output.
  P0 |= 0x40; // Output high
 */
static void simpleProfileChangeCB( uint8 paramID )
{
  ...
  switch( paramID )
  {
    case SIMPLEPROFILE_CHAR1:
     ...
     if(newValue==9){appForceBoot (); } //proshivka sbl     
      
      switch( newValue )
      {
        case 1: 
          advertData[7]=newValue;GAPRole_SetParameter( GAPROLE_ADVERT_DATA, sizeof( advertData ), advertData );
        break;         
        case 2: 
          advertData[7]=newValue;GAPRole_SetParameter( GAPROLE_ADVERT_DATA, sizeof( advertData ), advertData );
        break;      
